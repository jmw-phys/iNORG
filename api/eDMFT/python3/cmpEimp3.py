...
def ReadPARAMS2(fparams, fout, dir, solver):
    """
    Modified to adapt to the ed solver output.
    In case of ed solver, the cix file does
    not exist.
    """
    fp = open(fparams, 'r')
    ...
    if solver=='ed':
        for line in fp:
            ...
            dat = line.split()
            if len(dat)>=2:
                m = re.search('Delta|Gf|Sig', dat[0])
                if m is not None :
                    cmn = dat[0]+'="'+dat[1]+'"'
                    exec(cmn, globals())
                    print('executed:', cmn, file=fout)
                m = re.search('beta',dat[0])
                if m is not None :
                    cmn = dat[0]+'='+dat[1]
                    exec(cmn, globals())
                    print('executed:', cmn, file=fout)
        # for mu we have to search it in Sig
        fsig = open(dir+Sig,'r')
        mu = float(fsig.readlines()[0].strip().split()[2].strip().split('=')[1])
        fsig.close()
        print('beta=', beta, 'mu=', mu, 'cix=', ' ', 'Delta=', Delta, file=fout)
        return ('',Delta,Gf,Sig,beta,mu)
...
def read_DegEks(dir,fparams,mu):
    """
    Equivalent of ReadCix for ED solver. Output is:
     - Deg: number of correlated orbitals (spin included)
     - Eks: impurity energies (shifted by the chemical potential)
    """

    # Lets first search for Sigind
    ftrans = open(dir+'/Trans.dat','r')
    lines = ftrans.readlines()
    ftrans.close()               #extracted data, now close
    Sigind = array([int64(line.strip().split())
                       for line in lines[2:2+int64(lines[0].strip().split()[0])]
                       ])

    # if some off-diagonal terms
    if len([i for i in unique(Sigind-diag(Sigind)) if i!=0])!=0:
        print('WARNING: The impurity functional cmpEimp2.py works for now only'
              +' when baths are diagonal. You should not trust total free energy.'
              +' Needs some development...!')

    # number of different entries in Def dictionnary
    nDegelem = len([i for i in unique(diag(Sigind)) if i!=0])
    # we need to know how much orbitals for each different entries
    nelem = []
    for ind in range(nDegelem):
	    nelem.append(diag(Sigind).size - count_nonzero(diag(Sigind)-ind-1))

    # Check if spin dependent
    fp = open(fparams,'r')
    lines = fp.readlines()
    fp.close()
    varnames = array([line.strip().split()[0] for line in lines]) #all variable names
    spin_dep = eval(lines[where(varnames=='spin_dependent')[0][0]].strip().split()[1])

    Deg={}
    # if not spin dep, multiply all entries by 2
    if not spin_dep:
        for ielem in range(nDegelem):
            Deg[ielem]=nelem[ielem]*2
    # no otherwise
    else:
        for ielem in range(nDegelem):
            Deg[ielem]=nelem[ielem]

    # read one-body matrix given to ed solver
    with  open(dir+lines[where(varnames=='EDinput')[0][0]].strip().split()[1], 'r') as f:
        F = f.read()
        I = F.find('Tij')
        if I==-1:
            raise ValueError('The string "Tij" is not found in the file')
        I = F.find('\n', I)+1
        J = F.find('\n\n', I)
        if J == -1: len(F)
        Flines = F[I:J].splitlines()
        Eimp = genfromtxt(Flines, invalid_raise=True, loose=False, dtype=float128)
        if not bool(Eimp.shape):
            Eimp = ones(1)*Eimp
        else:
            Eimp = diag(Eimp)
        Eks = []
...
def GiveImpFunctional(dire, fparams, Vdc, fout=sys.stdout, solver='ctqmc'):
    """
    Modifications here so to accomodate ed solver. The new input solver
    is initialized to ctqmc to avoid troubles with ctqmc.py, and allows
    to specify the ed solver.
    """
    ...
    (cix,Delta,Gf,Sig,beta,mu_qmc) = ReadPARAMS2(dire+'/'+fparams, fout, dire, solver)
    if solver=='ctqmc':
        (Deg, Eks) = ReadCix(dire+'/'+cix, fout)
    elif solver=='ed':
        (Deg, Eks) = read_DegEks(dire,dire+'/'+fparams,mu_qmc)
    else:
        raise ValueError("Solver not recognized in GiveImpFunctional, please provide a choice betweem: ctqmc, ed")
    ...
    print(Eks,mom[:lengh])
    print('1/2*Tr(Sigma*G)=', 0.5*(trsigg+trsigg_oo), 'deimp*nf=', sum(Eks*mom[:lengh]), file=fout)

    Vdc_nd = sum(array(mom[:])*array(Vdc[:]))
    zeorb = TrDeltaG+Epot-cxx - sum(Eks*mom[:lengh]) # note that Epot ~ 1/2*Tr(Sigma*G)+Eks*mom
