...
def format_norg_param(v):
    """Format parameter for norg PARAMS file, handling numpy arrays and lists."""
    if isinstance(v, (list, ndarray)):
        # Convert numpy arrays to lists of python types
        if isinstance(v, ndarray):
            v = v.tolist()
        # Format list to string '[1, 2, 3]' and remove commas for norg
...
class IMP_NORG:
    ...
    def __init__(self, Znuc, dire, params, Sigind, CF):
        ...
        # parameters, which will be given to norg and will be written to PARAMS for norg execution
        # This is just a default set of paramerets, which will be modified before the execution
        self.sparams={'exe':'norg',
                      'Sig':'Sig.out',
                      'Delta':'Delta.inp',
                      'cix':'actqmc.cix',
    ...
    def _exe(self, params, DCs, extn, UpdateAtom, gbroad=0.0, kbroad=0.0, maxAc=2000.):
        ...
        # --- norg-specific Ed & Deg ------------------------------------------------
        norg_eimps_all = self.Eimps.flatten()
        nband = 2 * self.l + 1  # d-shell: l=2 → nband=5

        # Detect magnetic: explicit params setting takes priority, Sigind auto-detection as fallback
        if 'magnetic' in params:
            is_magnetic = bool(params['magnetic'][0])
        else:
            diag_sigind = [self.Sigind_shifted[i,i] for i in range(len(self.Sigind_shifted))]
            n_unique = len(set(s for s in diag_sigind if s > 0))
            is_magnetic = (n_unique == 2 * nband)
        self.is_magnetic = is_magnetic

        if is_magnetic:
            # Magnetic: Ed has nband values (per-band), average up/dn crystal field
            # Eimp ordering from eDMFT: [orb1_up, ..., orb5_up, orb1_dn, ..., orb5_dn]
            norg_eimps = array([0.5*(norg_eimps_all[i] + norg_eimps_all[i + nband])
                                for i in range(nband)])
            norg_degs = list(range(1, nband + 1))  # [1, 2, 3, 4, 5]
            params['magnetic'] = [1, "# spin-resolved magnetic calculation"]
        else:
            # Non-magnetic: original behavior
            norg_eimps = norg_eimps_all
            norg_degs = list(range(1, len(norg_eimps) + 1))
        ...
        # Compute nmesh (number of Matsubara frequencies in Delta.inp) for C++ solver
        T_mesh = 1./params['beta'][0]
        m_max_mesh = int(om[-1]/(pi*T_mesh)+1e-6)
        nmesh_val = len(arange(1, m_max_mesh+1, 2))
        params['nmesh'] = [nmesh_val, "# number of Matsubara frequencies in Delta.inp"]

        # Preparing params.dat file
        with open(self.dir+self.PARAMS, 'w') as fp:
            for p in ['Ed', 'Deg', 'J', 'CoulombF', 'beta', 'fit_range', 'fit_points', 'fit_nbaths', 'U', 'NOOC', 'restrain', 'distribute', 'Minsulator', 'ful_pcl_sch', 'weight_nooc', 'weight_freze', 'magnetic', 'nmesh']:
                if p in params:
                    val_str = format_norg_param(params[p][0])
                    print("%s  %s " % (p, val_str), '\t ', params[p][1], file=fp)
        ...
        print('Running ---- norg -----', file=self.fh_info)
    ...
    def HighFrequency(self, params, DCs, nl_imp, extn, wbroad=0.0, kbroad=0.0):
        ...
        if hasattr(self, 'is_magnetic') and self.is_magnetic:
            # Magnetic: keep separate spin-orbital occupancies
            # nmat.txt format: 1up val, 1dn val, 2up val, 2dn val, ... (interleaved)
            mom_interleaved = []
            i = 0
            while i < len(lines):
                line = lines[i].strip()
                if not line or line.startswith('sup') or line.startswith('sdn') or line.startswith('sum'):
                    break
                mom_interleaved.append(float(line.split()[1]))
                i += 1

            # Reorder from interleaved (up-dn-up-dn) to block (5up+5dn) matching Sigind order
            nband = len(mom_interleaved) // 2
            mom = [mom_interleaved[2*j] for j in range(nband)] + \
                  [mom_interleaved[2*j+1] for j in range(nband)]
        else:
            # Non-magnetic: sum up/dn pairs (original behavior)
            i = 0
            while i < len(lines):
                line = lines[i].strip()
                if not line or line.startswith('sup') or line.startswith('sdn') or line.startswith('sum'):
            ...
            Deg = params['Deg'][0]
            Max_Deg = max(Deg)
            filtered_mom = [0.0] * Max_Deg
            for deg, m_val in zip(Deg, mom):
                filtered_mom[deg - 1] += m_val
            mom = filtered_mom
        
        # Process the remaining lines to get ntot
        for line in lines[i:]:
            if line.strip().startswith('sum'):
                nf = float(line.strip().split()[1])
                break
        
        ntot = nf
        TrSigmaG = 0 # Should be properly computed if needed
        #!#####################################################################
...
if __name__ == '__main__':
    ...
    imp = IMP_NORG(23, dire, iparams0, Sigind, CF)
