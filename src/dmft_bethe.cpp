/*
code developed and maintained by (jmw@ruc.edu.cn, RUC, China) date 2022
*/
//! not finished yet 
	...
	gloc_err({ 1.e99,1.e98,1.e97 ,1.e96 ,1.e95}), se(p.nband, p), imp_backup(p.imp_backup)
{
    ...
    // Try to read parameters from PARAMS.norg first
    // if (IFS("PARAMS.norg")) {
    //     read_params_norg("PARAMS.norg");
	// 	// g_loc. = ImGreen(p.nband, p);
	// 	// if(mm) WRN("HERE is fine 1-32"+NAV2(p.nband, g_loc[0]))
	// 	// se = ImGreen(p.nband, p);
	// 	// n_eles.reset(VecReal(p.norbs, 0.));
    // } else {
        set_parameter();
    // }
	...
	NORG norg(mm, p);
	log("initial");	
	if (fitdata) 	bth.bth_read_fvb("ose_hop");
	...
	while (iter_cnt < p.iter_max && !converged()) 
	{
		...
		if (!(fitdata && iter_cnt == 1)) {
			...
			bth.number_bath_fit_all(hb);													if (mm) bth.bth_write_fvb(iter_cnt);
		}
