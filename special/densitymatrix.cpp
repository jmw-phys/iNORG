...
VEC<MatReal> DensityMat::find_unitary_orbital_rotation_matrix()
{
	...
	if(p.if_norg_imp){
		...
		for_Int(spin, 0, 2) rotaionU_bath[0 + spin] = rotaionU_bath[2 + spin] = 0.5 * (rotaionU_bath[0 + spin] + rotaionU_bath[2 + spin]); //! set band 0 and band 1 were same.
...
void DensityMat::update(Int mode) {
	dm = dm_initialize();
	if (mode == 1) {
		MatReal egses(lowest_eigpairs(scsp.dim, false, p.degel + 1));
