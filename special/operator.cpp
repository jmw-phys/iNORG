...
// Operator::Operator(const MyMpi& mm_i, const Prmtr& prmtr_i, const Tab &tab, const NocSpace& s_i):
// 	mm(mm_i), p(prmtr_i), scsp(s_i), table(tab), dim(s_i.dim), oper_value(pow(p.norbit, 4) + pow(p.norbit, 2) + 1)
// {
// }
	...
	for_Int(h_i, row_H.bgn(), row_H.end()) {
		...
		for (const auto &x : a.filled_spinless) {
			for (const auto &i : x) {
				...
				for_Int(pos, 0, 3) h_idxs.push_back(h_idx[pos]);
			}
		}
/*
		...
		for_Int(set_i, 0, p.norg_sets) {
			for_Int(set_j, 0, p.norg_sets) {
				...
				for (const auto& N_i : N_veci) for (const auto& N_j : N_vecj) {
					// if (N_i != N_j) {
						h_idx = { sparse_idx, h_i, int(mat_hop_pos.size() + 1 + tensor_u[N_i][N_j][N_j][N_i]) }; for_Int(pos, 0, 3) h_idxs.push_back(h_idx[pos]);
				...
				for (const auto &i : off_dt_next){
					...
					if (p.if_norg_imp) for (const auto& x : a.filled_spinless) {
						for (const auto& N_s : x) if (N_s != i[0] && N_s != i[1]) {
							//																		[i   ][j  ][k  ][l   ]
							h_idx = { sparse_idx, i[2],-i[3] * int(mat_hop_pos.size() + 1 + tensor_u[N_s][i[1]][N_s][i[0]]) }; for_Int(pos, 0, 3) h_idxs.push_back(h_idx[pos]);
							h_idx = { sparse_idx, i[2], i[3] * int(mat_hop_pos.size() + 1 + tensor_u[N_s][i[1]][i[0]][N_s]) }; for_Int(pos, 0, 3) h_idxs.push_back(h_idx[pos]);
							h_idx = { sparse_idx, i[2], i[3] * int(mat_hop_pos.size() + 1 + tensor_u[i[1]][N_s][N_s][i[0]]) }; for_Int(pos, 0, 3) h_idxs.push_back(h_idx[pos]);
							h_idx = { sparse_idx, i[2],-i[3] * int(mat_hop_pos.size() + 1 + tensor_u[i[1]][N_s][i[0]][N_s]) }; for_Int(pos, 0, 3) h_idxs.push_back(h_idx[pos]);
						}
					}
				}
			}
...
SparseMatReal Operator::find_hmlt(const Tab h_idx) const
{
	...
	Real diagonal(0.), h_pick_up_value(1e-5);	Bool flag(false);
	for_Idx(pos, 0, h_idx.size()/3)
	{
		Int coeff_comm = h_idx[2 + pos*3] >= 0 ? 1 : -1;
		if (h_idx[1 + pos*3] == h_idx[0 + pos*3] + row_H.bgn()) {diagonal += coeff_comm * oper_value[(abs(h_idx[2 + pos*3]))];flag=true;}
		else {
			if (flag) {hmlt_splited.addelement(diagonal, h_idx[1 + (pos-1)*3], h_idx[0 + (pos-1)*3]); diagonal = 0.;flag=false;}
			// if (ABS(oper_value[(abs(h_idx[2 + pos*3]))]) > h_pick_up_value) // Picking up significant values in H.
				hmlt_splited.addelement(coeff_comm * oper_value[(abs(h_idx[2 + pos*3]))], h_idx[1 + pos*3], h_idx[0 + pos*3]);
		}
	}
...
// void Operator::save_the_Tab(Tab& tab, Str name) const{
	// Int size_temp(tab.size()/3);
	// Int size = mm.Allreduce(size_temp);
	// VecInt v_size_i(1); v_size_i[0] = tab.size()/3;
...
void Operator::write_the_Tab(const std::string& filepath) const {
	const std::vector<int>& data = table;
    FileHeader header;
    header.buffer_size = data.size() * sizeof(int);
    ...
    // Write data
    file.write(reinterpret_cast<const char*>(data.data()), header.buffer_size);

    file.close();
}

VEC<int> Operator::read_the_Tab(const std::string& filepath) const {
	VEC<int> data;
    ...
    // Resize vector based on data count in header
    data.resize(header.data_count);

    // Read data
    file.read(reinterpret_cast<char*>(data.data()), header.buffer_size);
